...
\begin{defn}
    The greatest common divisor of integers \(a\) and \(b\), not both zero, is a positive integer
    \(d\) such that \(d\) divides both \(a\) and \(b\), and \(d\) is greater than or equal to every
...
\begin{lemma} \label{lemma_div2}
    ...
    \begin{proof}
        ...
        Thus we have shown that if \(n\) is odd, then \(n^2\) is odd.
...
proved that \(e\) is transcendental in 1873, and Ferdinand von Lindemann proved that \(\pi\) is
transcendental in 1883 \cite{transcendental_wiki}.


\bibliographystyle{plain}
\bibliography{refs}

\includepdf[pages=-]{sqrt2_feedback.pdf}
