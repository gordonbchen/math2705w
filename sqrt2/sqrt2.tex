...
\begin{thm}
    ...
    \begin{proof}
        ...
        is in lowest terms, meaning that \(\gcd(m, n) = 1\). Then \((\frac{m}{n}) ^ 2 = 2\), so
        ...
        but found that both are divisible by
...
We know that the square root of any prime number is irrational by generalizing
Lemma \ref{lemma_div2} into Euclid's Lemma \cite{euclid_wiki}.
...
The Pythagoreans are credited with the discovery that \(\sqrt{2}\) is irrational \cite{sqrt2_wiki}.
It was said that Hippasus of Metapontum was drowned at sea for telling those not in the
